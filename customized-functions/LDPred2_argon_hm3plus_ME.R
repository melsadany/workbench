...
#### extended function ####
calculate_ldpred_pgs <- function(sumstats, n_core, plink_rds, sd_y = 1, pheno_name = 'pheno_not_given', build = 'hg19', output_path){
  ...
  map_ldref <- readRDS("/Dedicated/jmichaelson-wdata/lcasten/tools/LDPred2/map_hm3_plus.rds")
  if(build == 'hg19'){
    map_ldref <- map_ldref %>% 
  ...
  for (chr in 1:22) {
    ...
    corr_chr <- readRDS(paste0("/Dedicated/jmichaelson-wdata/lcasten/tools/LDPred2/ldref_hm3_plus/LD_with_blocks_chr", chr, ".rds"))[ind.chr3, ind.chr3]
    if (chr == 1) {
